...
export interface paths {
    ...
    "/notifications": {
         ...
         * @description Returns the account's in-app notification feed, newest first (PRD §7.5 NOT-001, §17 bounded reads). Each item carries the SHARED product event id — the same id the daily email digest references, so a notification is one event on two surfaces, never two. Items reference locale catalog KEYS with named slots (LOC-002); the surface renders copy, the core stores none. This is a read. The feed is BOUNDED and keyset-paginated over `(createdAt, id)` newest-first: pass `limit` for the page size (server default when omitted, clamped to a hard maximum) and the opaque `cursor` from a prior response's `nextCursor` to fetch the next page. Ties on `createdAt` break by `id` so every notification is returned EXACTLY ONCE across pages; a newer insert appears only on a refreshed first page, never duplicated mid-scroll.
         */
...
export interface components {
    schemas: {
        ...
        BulkApprovalConfirmResult: {
            ...
            valid: boolean;
            /** @description True only while at least one member carries a LIVE, still-unresolved execution authorization (approved / revalidating / executing). False once every member's write has produced an external result, even though those members still report `already_authorized`. */
            executionPending: boolean;
        ...
        /** @description The in-app notification feed for an account (NOT-001), newest first, with the current account-wide unread count for the badge and bounded keyset pagination (§17). `hasMore` reports whether older notifications exist beyond this page; `nextCursor` is the opaque continuation token to pass back as `cursor` to fetch them (null on the last page). `unreadCount` stays the ACCOUNT-WIDE badge count, not the count within the returned page. */
        NotificationFeed: {
            /** Format: uuid */
            marketplaceAccountId: string;
            /** Format: int64 */
            ...
            /** @description True when older notifications exist beyond this page (an additional page can be fetched with `nextCursor`). False on the last page. */
            hasMore: boolean;
            /** @description Opaque keyset continuation token for the next (older) page, or null when `hasMore` is false. Pass it back verbatim as the `cursor` query param. */
            nextCursor?: string | null;
        };

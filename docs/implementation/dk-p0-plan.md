...
| Review agents | `.claude/agents/*.md` (10 agents incl. read-only `safety_release_reviewer`) | Review routing exists and is wired into the orchestrator (see §4.6) |
...
### 4.6 Review routing & execution driver — in-context orchestrator using the repo's 10 agents
**Decision (2026-07-16):** execution uses the **in-context subagent orchestrator** (`dk-p0-orchestrator-prompt.md`), not a background workflow, because this change is gate-heavy (multiple human sign-offs, live probes, paid eval runs) and the repo already ships area reviewers. Diff-type → reviewer mapping: contracts/`gen` → `api_data_contracts`; Go connector/observation/scheduler → `go_connector_observer`; Go money/policy/approval/execution → `go_domain_executor`; Python plane → `python_llm_evals`; SPA → `web_frontend`; extension → `chrome_extension`; locale/copy/RTL → `persian_localization_ux`; deploy/observability/jobs → `platform_reliability`; **every phase-boundary merge and every gated step additionally reviewed by `safety_release_reviewer`** (read-only); `product_delivery_lead` consulted at phase gates for §4.6-descope/schedule calls.
Why: the agents encode the PRD's own review discipline; the safety reviewer is deliberately independent. Trade accepted: slower than a fully parallel background workflow; correct for a change with this many human gates.
...
Sources (verified in-repo 2026-07-16): `docs/PRD.md`; `docs/DK Marketplace - Open API Service.yml` (size/presence); `docs/DK-public-research-result/01–14`; `design/README.md`, `design/IA_AND_COMPONENTS.md`, `design/LOCALIZATION.md`, `design/STATE_MATRIX.md`, `design/FLOWS.md`; `.claude/agents/*.md` (10 files).

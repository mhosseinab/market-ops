...
**Amendment (2026-07-17, product owner):** the LLM plane adopts a single-ecosystem two-layer agent stack — **LangGraph** as the only top-level orchestrator (workflow state, branching, parallelism, node-level retry) and **LangChain `create_agent`** for individual agents embedded as leaf-level graph nodes/subgraphs (prompts, model access, tools, typed outputs via `response_format` Pydantic models returning a validated `structured_response`). One ecosystem means agents and graph share message formats, runtime context, tool definitions, streaming events, and LangSmith traces — no cross-framework translation layer. Post-P0 direction is multi-agent: specialist agents (pricing analysis, listing diagnostics, …) join as nodes without re-architecture. The boundary discipline above is otherwise unchanged, with these bindings: the frameworks are confined to `services/llm`; model access remains OpenAI-compatible-only through `langchain-openai` `ChatOpenAI(base_url=…)` — base URL/credential/model are configuration, no provider-specific SDK branches; framework types never enter canonical contracts, `gen/*`, or the Go core; graph state stores JSON-safe business data only, never agent/client/runtime objects; the Draft-only tool registry stays the single source of tools any agent may bind, and its negative tests are unchanged; `response_format` models carrying money use mantissa/currency/exponent or raw evidence strings — never float (§9.1 applies inside the LLM plane too). Two deliberate rejections of the generic LangGraph pattern: (1) approval is **never** a graph interrupt/human-in-the-loop resume — the graph's terminal write is at most a Draft, and approval happens outside the model plane via the structured control (PRD §8, §12.3); (2) no durable checkpointer ships in P0 (the LLM plane holds no DB credential, §19.3) — graph state is per-request in-process, conversation durability stays in the gateway's tables, and `langgraph-checkpoint-postgres` becomes an option only if that credential decision is deliberately revisited. The single §12.4 transient retry lives at graph/node level — never stacked with another retry mechanism on the same failure.

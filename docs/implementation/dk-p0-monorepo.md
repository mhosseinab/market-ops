...
- `deploy/compose.test.yml` (S32, `task test:integration`): the minimal REAL stack the cross-plane suite needs — postgres, mockdk, core and llm run via `go run`/`uv run uvicorn` against the mounted repo (no prod image yet; that is S34's distroless/uv build), the built `apps/web/dist` served statically, and a Caddy ingress (`deploy/caddy/Caddyfile.test`) unifying web+gateway under one origin exactly like `compose.prod.yml`'s topology — required so browser-driven tests see real cross-plane responses instead of a same-origin-only false green. No otel/Grafana/Mailpit/Spotlight (that is `compose.dev.yml`'s concern).

...
│   ├── compose.prod.yml      # prod topology: nginx (TLS ingress), core, llm, postgres, backup job
│   └── nginx/, grafana/      # configs
...
tasks:
  ...
  images:build/validate: # build all FOUR published images (core, llm, nginx, goose) locally + inspect non-root/entrypoint/OCI labels/no baked secrets; never deployed
  release:images: # copy the release run's digest-pinned *_IMAGE lines into $ENVFILE (tools/deploy/release_images.sh); digest-only, writes $ENVFILE.prev-images
  prod:*:      # fail-closed wrappers over `docker compose --env-file "$ENVFILE" -f deploy/compose.prod.yml` (config, certs:issue/renew, migrate[:status], up, ps, logs, rollback)
...
| Release pinning | `task release:images -- --tag vX.Y.Z --env-file "$ENVFILE"` then `task release:images:check` | four `@sha256:` refs in `$ENVFILE`; non-digest input rejected (`tools/deploy/release_images_test.sh`) |
| Production parse | `task prod:config` | exit 0, pass/fail only — the rendered config is never printed (it holds every substituted secret) |
...
ledger:    ['docs/implementation/dk-p0-progress.md', 'tools/ledger/**', 'Taskfile.yml', '.github/workflows/**']
```

Jobs (each `if:` its filter): **contracts** — `task contracts:generate` + `git diff --exit-code` (the drift check); **go** — setup-go with `go-version-file: services/core/go.mod`, `GOWORK=off` explicit on every step, golangci-lint, `go test ./... -race`, scratch-Postgres service container for DB tests (goose up/down assertions run here); **py** — `astral-sh/setup-uv`, `uv sync --frozen --group dev`, ruff, mypy from root, pytest; **ts** — pnpm `--frozen-lockfile`, biome, typecheck, vitest, extension + web builds, then a Chromium install + `task ts:pseudoloc` (pseudo-locale + copy-lint gate, LOC-011, incl. the browser layout pass `pseudoloc:visual` from #15); **deploy-obs** (from S33) — installs the REQUIRED validators (`promql-parser`, `PyYAML`, pinned, no `|| true`) then `task obs:validate` (§18 dashboard drift + real PromQL parse of every dashboard AND alert expr + series-name check against the single `deploy/obs/metrics_inventory.json` + §20.1 alert-rule/runbook checks). The gate FAILS CLOSED (issue #156): a missing validator or an unknown series is a job failure, never a silent skip-to-success. **ledger-integrity** (from #203, gated on the `ledger` filter above) — `actions/setup-python@v6` then `bash tools/ledger/validate_ledger_test.sh` (the RED+GREEN fixture regression suite) and `python3 tools/ledger/validate_ledger.py --file docs/implementation/dk-p0-progress.md` against the REAL checked-out ledger; the validator is stdlib-only, so no dependency install. This is the REMOTE enforcement point for the never-cut ledger-integrity rule (issue #19): before #203 a ledger-only or validator-only PR completed with only `detect` and no ledger check, so a malformed ledger could merge green. It FAILS CLOSED (no `|| true`): a `passed` step with a still-pending mandatory gate, an unclassified deferred bullet, or a broken negative fixture is a job failure. NOTE: for the check to be non-bypassable it must also be added to `main`'s required-status-checks list in branch protection (repo-admin action, not settable from the workflow file). A contracts change triggers the four language jobs. `task ci:local` mirrors this exactly for the pre-merge loop.
...
- `deploy/compose.prod.yml`: the single-VPS topology. Every service runs a digest-pinned image published by `.github/workflows/release.yml`; nothing mounts repo source. `nginx` is the SINGLE ingress and the only service publishing host ports — it terminates TLS (`deploy/nginx/nginx.prod.conf`: 443 + HSTS, an HTTP→HTTPS redirect, the ACME challenge path, and an unpublished plain 8080 listener the healthcheck uses), bakes the SPA at `VITE_GATEWAY_BASE_URL=/api`, and proxies `/api/` to core, so the browser sees one origin. Certificates are issued and renewed by a one-shot `certbot` service behind a `certs` compose profile, pinned to the domain-independent `--cert-name market-ops` because the image's own `ENTRYPOINT` bypasses the base image's envsubst step. Then `nginx` → `core` (distroless static Go image) → `postgres:18` (volume on `/var/lib/postgresql`, the 18+ major-version parent, NOT the legacy `.../data`). `llm` (uv `--no-editable --frozen` image, build context = repo root) sits alongside core and deliberately receives no `DATABASE_URL` and no `CONNECTOR_ENCRYPTION_KEY` (§8/§12.3 — a never-cut invariant, not a convenience). A one-shot `migrate` service (`deploy/goose.Dockerfile`) applies the goose app schema before core starts and is **forward-only** — entrypoint `goose`, command `up`, `restart: "no"` — because `cmd/core` applies only River's own internal schema at boot. `core` carries no container healthcheck (distroless: no shell); the `nginx` healthcheck hits `/api/healthz` through the proxy, so `up -d --wait` gates on the real edge→gateway path. Operators never hand-assemble the four `*_IMAGE` values: `.github/workflows/release.yml` uploads them as `images.env` inside the `image-digests-<version>` artifact on a published tag, and `task release:images` (`tools/deploy/release_images.sh`) merges them into the out-of-checkout `$ENVFILE` — rejecting any non-`@sha256:` reference and saving the replaced digests to `$ENVFILE.prev-images` for `task prod:rollback`. A pull-request run builds and Trivy-scans the same eight matrix jobs but skips publication entirely, so it produces `.dockerbuild` records and **no** images; DEPLOYMENT.md §6.1 spells that out because a green PR run reads like a release otherwise. **Not yet present:** WAL archiving to the isolated backup destination and the otel stack — both are outstanding S34 items, tracked in `DEPLOYMENT.md` §9.
- `deploy/compose.test.yml` (S32, `task test:integration`): the minimal REAL stack the cross-plane suite needs — a one-shot `migrate` service applies the goose app-schema migrations + River's own schema + the dev seed (`task db:reset`, since `cmd/core` only runs River's OWN internal schema, never the goose app schema) before postgres-dependent services start; mockdk, core, and llm run via `go run`/`uv run uvicorn` against the mounted repo (no prod image yet; that is S34's distroless/uv build); the built `apps/web/dist` is served statically; and an Nginx ingress (`deploy/nginx/nginx.conf`) unifies web+gateway under one origin exactly like `compose.prod.yml`'s topology — required so browser-driven tests see real cross-plane responses instead of a same-origin-only false green. The integration stack terminates no TLS: production runs the same Nginx layer with `nginx.prod.conf` mounted over it, which adds only the TLS listener, the redirect, and the ACME path. No otel/Grafana/Mailpit/Spotlight (that is `compose.dev.yml`'s concern).

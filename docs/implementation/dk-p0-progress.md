...
| S8 | AuthN/Z + shared permission matrix [C] | in_progress | 1 | (worker worktree) | — | dispatched (domain-execution worker; +SECURITY co-review — argon2id creds/sessions/perm matrix). [C]: adds auth endpoints+regen. Migration 0003. DB market_ops_s8. Perm matrix ONE source for chat+screens (ACC-002 negative tests per role×level) |

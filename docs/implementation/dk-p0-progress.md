...
| S8 | AuthN/Z + shared permission matrix [C] | in_progress | 1 | worktree-agent-ab0b9b74 | f7329bc | worker GREEN: ONE perm.Can matrix (chat+screens, must-deny coverage per role×L1-L4), argon2id (m=19MiB) + server-side sessions (256-bit token, Secure/HttpOnly/SameSite cookie, only SHA-256 stored, never in body), fail-closed middleware every non-public route, contract additive (login/me/logout+cookieAuth, drift 0), migration 0003 reversible, ci:local exit 0. Ships dark. DUAL review: domain-execution area + safety_release SECURITY. CF: matrix pre-declares later-step actions (S12/S16/S17/S20 must authorize via perm.Can); new `session` contract tag (additive). Merge overlap w/ S10: go.mod + migration numbering (0003 vs 0004). Merge BY SHA f7329bc |
...
| S10 | Catalog + owned-offer sync | in_progress | 1 | worktree-agent-a0da8ff9 | 4ceaa33 | worker GREEN: migration 0004 (products/variants/listings/owned_offers + append-only snapshots + sync_runs), resumable/idempotent River sync + reconciliation, owned-offer price = RawAmount (no Money path), reorder/replay zero-dup + resume + pagination-fault tests pass, migrate:verify up/down/up clean, no contract/gen change. In review (connector-observation). CF: catalog.RegisterWorkers not yet wired into cmd/core (bootstrap step); products.brand_title empty until enrichment; reconciliation report-only (never deletes). Merge overlap w/ S8: migration numbering (0003 S8 / 0004 S10) + go.mod |

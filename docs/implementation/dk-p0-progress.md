...
| S4 | Contracts pipeline + drift check [C] | in_progress | 1 | worktree-agent-aa8891ee | b9d2997 | attempts 1&2 aborted (stale-base, not review fails). Attempt-3 worker GREEN: gateway.openapi 3.1 v0 + gen/{go,dkgo,python,ts} committed, pinned generators (oapi-codegen v2.7.2, openapi-ts 7.13.0, openapi-python-client 0.29.0), deterministic DK normalizer→temp (frozen doc untouched), drift check in ci:local+lefthook pre-push. All Verify pass. In review (contract-data). MERGE NOTES: edits services/core/go.mod (require+replace gen/go,gen/dkgo — overlap w/ S5 already-merged require block → resolve at merge), lefthook.yml (exclude gen/), .golangci.yml (gen exclusion + connector depguard), Taskfile.go.yml (go.work += gen/go gen/dkgo). Merge BY SHA b9d2997 |

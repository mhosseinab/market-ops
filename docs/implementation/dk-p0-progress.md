...
Review routing: per plan §4.6 — api_data_contracts (contracts/gen) · go_connector_observer (connector/catalog/identity/observation/routec) · go_domain_executor (money/policy/approval/execution/audit/outcome) · python_llm_evals (services/llm) · web_frontend (apps/web) · chrome_extension (apps/extension) · persian_localization_ux (locale/copy/RTL/Persian fixtures) · platform_reliability (deploy/CI/observability) · **safety_release_reviewer at phase boundaries (after S7, S19, S24, S29, S31, S33) and all gated steps** · product_delivery_lead for schedule/descope calls.

...
| S6 | CI pipeline | in_progress | 1 | worktree-agent-ad87289d | f0c21df | worker GREEN: ci.yml (detect+contracts+go+py+ts jobs, paths-filter, GOWORK=off, pg18 service container GitHub-only); added `migrate:verify` (up/down/up reversibility) + ts:build to ci:local; pseudoloc placeholder for S25. ci:local exit 0 + actionlint exit 0. In review. Merge overlap: root Taskfile.yml ci:local w/ S7 (additive, resolve at merge). First-GitHub-run = deferred gate |

...
| S10 | Catalog + owned-offer sync | in_progress | 1 | (worker worktree) | — | dispatched (connector-observation). NOT [C] — persists sync-status in DB, NO contract change. Migration 0004. DB market_ops_s10. Owned-offer price = quarantined RawAmount (S7), NEVER Money; reorder/replay→zero dups; append-only raw snapshots |

...
| S6 | CI pipeline | in_progress | 1 | worktree-agent-ad87289d | f0c21df | worker GREEN: ci.yml (detect+contracts+go+py+ts jobs, paths-filter, GOWORK=off, pg18 service container GitHub-only); added `migrate:verify` (up/down/up reversibility) + ts:build to ci:local; pseudoloc placeholder for S25. ci:local exit 0 + actionlint exit 0. Cycle 1: CHANGES_REQUESTED — BLOCKER: ci.yml pinned golangci-lint v1.64.8 but repo uses .golangci.yml version:"2" + go 1.25.0 → v1.x binary hard-errors on first GitHub run (reviewer reproduced); local ci:local masked it via ambient v2.5.0. Fix cycle 2 = 138b85e (cherry-picked f0c21df→6bfd7ca + pin golangci-lint v2.5.0; proved v2.5.0 lints clean; ci:local+actionlint exit 0). In review cycle 2. Merge overlap: root Taskfile.yml ci:local w/ S7 (additive, resolve at merge). First-GitHub-run = deferred gate |
| S7 | Money type + static guard | in_progress | 1 | (worker worktree) | — | dispatched (domain-execution). **Phase-A boundary → safety_release invariant review required BEFORE merge.** Money never-cut invariant |

...
- Every model provider is assumed to expose an OpenAI-compatible API. The LLM plane's agent stack is single-ecosystem two-layer (plan §4.8 amendment 2026-07-17), confined to `services/llm`: LangGraph is the only top-level orchestrator; LangChain `create_agent` defines individual agents as leaf-level graph nodes with typed outputs via `response_format` Pydantic models; model access uses `langchain-openai` `ChatOpenAI(base_url=…)` against a configurable OpenAI-compatible endpoint (base URL, credential reference, model, timeouts, and qualified capability settings are config). Do not add parallel provider-specific SDK adapters or leak framework/provider types into application/domain contracts.
...
- Docker Compose, Nginx, PostgreSQL, River, and the selected VPS topology are P0 delivery choices, not domain dependencies. Core behavior uses configuration and owned ports so packaging or infrastructure can change without rewriting business rules.

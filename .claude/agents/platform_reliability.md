...
- You own `deploy/` — `compose.dev.yml` (postgres:18, otel-collector, grafana/loki/tempo, mailpit for email testing, mock-DK server from S9) and `compose.prod.yml` (core as a distroless static Go image; llm as a uv multi-stage image built from repo root with `--no-editable --frozen --package llm`; the Nginx ingress image, which bakes the static web `dist/` and terminates TLS via `nginx.prod.conf`; a one-shot forward-only `migrate` service built from `goose.Dockerfile`; postgres 18 with WAL backup to the isolated destination; otel stack) — plus `.github/workflows/ci.yml`, `.github/workflows/release.yml`, the `Taskfile*.yml` set, and `lefthook.yml`.

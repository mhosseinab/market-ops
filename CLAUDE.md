...
- **No step IDs in source.** Filenames, test names, and code comments carry **durable** identifiers only — PRD requirement IDs (`LST-001`, `OBS-005`), `§` section refs, `PD-<n>` product decisions, issue numbers, and review-finding IDs (`S8-AUTHZ-001`). Orchestration step numbers are scheduling scaffolding: they belong in `docs/implementation/`, never in a module boundary or a doc comment. The one exception is a step that names a still-OPEN gate (S24/S34/S35) — that reference resolves to something, so it stays until the gate closes. Name a file for the domain it serves, not the step that produced it.

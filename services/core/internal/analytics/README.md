...
A **malformed** key is rejected too (`ErrMalformedDedupKey`). Precisely three segment shapes are rejected, and no others: a segment that is **empty** (`"briefing:daily_digest_sent:"`), a segment that is **blank after trimming** (`"briefing:daily_digest_sent: "`, `"…:\t"`), and a segment equal to the **zero UUID**, padding ignored (`"briefing:daily_digest_sent:00000000-0000-0000-0000-000000000000"`). These are the shapes a producer reaches when its identifying value is absent — `""` from a string field, whitespace from an untrimmed input, and `uuid.UUID`'s zero value, which matters most because `uuid.UUID` is the dominant identifier type in this core and its zero value stringifies to all-zeroes rather than `""`. Each is a non-empty Go string, so each passes *both* the `ErrMissingDedupKey` check and the database's non-empty CHECK while being a **constant** per `(account, family, name)`: the first such event would win the account's slot and every later, genuinely different, business fact would be suppressed forever. An unkeyed event deduplicates nothing; a constant key deduplicates everything, which is strictly worse.

The check runs on **every** key, delimited or not — a single-segment key can be blank or a zero UUID just as easily. What it does **not** catch, stated so the guarantee is not overclaimed: any *other* non-identifying constant (a literal `"unknown"`, a fixed sentinel id) is indistinguishable from a real identifier at this boundary and passes. This closes the zero-value class, not the general "is this value actually identifying?" question. Note also the `:` delimiter is *documented* but not yet *escaped* — `DedupKey(f, n, "a:b")` still collides with `DedupKey(f, n, "a", "b")`; committed-row identifiers never contain `:`, and escaping is follow-on work.

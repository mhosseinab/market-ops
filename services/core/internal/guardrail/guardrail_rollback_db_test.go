...
// TestGuardrailSetRollsBackMutationWhenAuditAppendFails exercises the ROLLBACK
// branch of the mutation+audit atomic seam: a failure injected BETWEEN the
// settings upsert and the AUD-001 audit append (via a DB-level trigger that
...
// WHOLE transaction back. It asserts BOTH that the guardrail settings row is
// absent AND that no audit row was appended — the settings change is never
// recorded without its reproducible audit trail, and never persisted alone.
func TestGuardrailSetRollsBackMutationWhenAuditAppendFails(t *testing.T) {
	pool, q := newPool(t)
	account := seedAccount(t, q)
	svc := guardrail.NewService(pool)
	actor := audit.Actor{ID: "owner@x.io", Role: "owner", Surface: "screens"}

	installAuditInsertFault(t, pool, "guardrail_rollback", account)
	...
	// The settings row must NOT be persisted: the whole tx rolled back.
	if _, err := svc.Get(context.Background(), account); err == nil {
		t.Fatal("guardrail settings row persisted despite audit-append failure — rollback branch is broken")
	}

...
2. **Contract (`contract.py`):** Defines the `ResponseEnvelope` which explicitly splits out `observed_facts`, `dk_signals`, `seller_config`, `deterministic_calculations`, `missing_data`, `comparisons`, `tables`, `exposure`, and `model_inference`. It also handles `SectionScope` and `AvailabilityCatalog` to map tools/evidence strictly to valid sections. It additionally defines the canonical ambiguity picker (`PickerCard` / `PickerCardOption`, serialized as `{id, label, contextKind}` — CHAT-007): a **non-executable display object** with no action id, no parameter version, no context version, no expiry and no approval control, and `extra="forbid"` so none can be smuggled in. Its surrounding copy is catalog keys (`chat.picker.title` / `chat.picker.hint`); option labels are authoritative read DATA.

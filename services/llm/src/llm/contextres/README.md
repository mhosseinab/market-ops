...
* **`turn.py` — the wire adapter.** `TurnContext` is the typed, `extra="forbid"` reader for the context payload the Go gateway puts on a turn (`services/core/internal/httpapi/chat.go`, `httpLLMChat.StartTurn`). `ContextKind` is the WIRE enum (`product`, `event`, …, matching the gateway's `ConversationContextKind`); `ContextKind.to_context_type()` / `context_kind_for()` are its total bijection with the domain `ContextType`. An unknown or misspelled key is rejected, never dropped — dropping it would silently lose the turn's subject.

  **What the producer sends today (as of `8650569`) vs. what the reader models.** The gateway emits `kind`, `version` (a JSON **int**), `entity_id` when the binding is entity-bound, and the binding's tenant provenance `organization_id` / `account_id` read from the persisted conversation row (omitted, never zero-valued, when unrecorded). It sends nothing else. The remaining modelled fields — `references`, `time_phrase`, `business_timezone`, `week_starts_on`, `recommendation_version` — are honored by the resolver but **unpopulated by any producer**; they arrive with the gateway read seam (**sub-scope 108c**). `now` is stamped by this plane's HTTP transport, never by the producer. `tests/test_turn_context_resolution.py` §8b posts the literal producer payload and is the cross-boundary contract guard for this seam.

  **The payload is validated in the resolver, not at the transport.** `llm.app.ChatRequest.context` is deliberately `dict[str, Any]`: typing it there would let FastAPI 422 a malformed payload before `resolve_turn_context` ran, and the gateway reads any non-2xx as `provider_unavailable` — so a context contract mismatch would look like an LLM-plane outage, with no §12.4 structured failure, no deep link and no `llm_context_resolution_total` event. Rejection still happens, one layer in, where it is structured and observable.
